...
$-- (#26a269) used for `examples` blocks. Point it at the brand's link green,
$-- the one brand colour dark enough to stay readable in that role -- the
$-- accent (#6af0ad) is a light mint and would not. Must come after \usetheme,
$-- since smile defines `green` when the theme loads it. The rest of smile's
$-- palette (red/blue/…) is semantic, not brand, and is left alone.
\colorlet{green}{brandLink}
$endif$
$if(colortheme)$
